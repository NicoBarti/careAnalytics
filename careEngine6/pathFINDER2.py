...
def retreive(selection,seed, directory):
    #for runRow in range(0,selection.shape[0]):
        #seed = selection["seeds"].iloc[runRow]
        #dat = selection["data"].iloc[runRow]
        dat = selection["data"].loc[selection["seeds"] == int(seed)]
        retreived, simdata = retreiver(directory = directory, seed=int(seed))
...
def find_5_traj(data, window = '200'):
    ...
    for trajectory in data:
        if data[trajectory].loc[str(window)] in qs:
            lineNumers.append(int(trajectory))
    return lineNumers
...
def plot_5_lines(data,type, window = '200'):
    ...
    for line in lineNumers:
        print(f'plotting line seed {line} with H window 200: {typeData[line].loc['200']}')
...
allruns = pd.read_csv("/Users/nicolasbarticevic/Desktop/CareEngineAnalytics/data/pathFinder/provisionalAllRuns.csv")
...
for type in runGrups:
    ...
    for runRow in range(0,selection.shape[0]):
        thisSeed = selection.iloc[runRow]['seeds']
        simdata = retreive(selection = selection, directory = directory, seed = thisSeed)
        plotlines = simdata["H"].mean(0)[1:]
        selectedLRunsAverages[thisSeed] = plotlines
...
##TODO: change rowLine to seed in this code, so I can make sure I'm plotting the same line  in pathFinderGrids
