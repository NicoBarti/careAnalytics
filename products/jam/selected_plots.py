...
def main():
    ...
    for schedule in schedules:
        ...
        if SETTINGS['jaccards']:
            ...
            plot_temporal_series(ax = ax_diagon[1], data = seekJac.diagonal(offset=1, axis1=1, axis2=2), color=color, label=f'{label}, next cycle similarity', linestyle='solid', title = '', y_label = '')
            plot_temporal_series(ax = ax_diagon[0], data = treatJac.diagonal(offset=1, axis1=1, axis2=2), color=color, label=f'{label}, next cycle similarity', linestyle='solid', title = '', y_label = '')
