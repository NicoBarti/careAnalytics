...
def onePlot(ax: mpl.axes.Axes,dd: pd.DataFrame, plotVariable: str, color: str, low_h_cut: float = None, high_h_cut:float = None,
    ...
    return ax
def populate_axe(ax_dict, low_h_cut: float = None, high_h_cut:float = None,sub_title:str = "", filteredData:pd.DataFrame = None):
    """
    ...
    for k, ax in ax_dict.items():
        ...
        if k == 'p':
            ...
            ax.add_patch(AA)
            AA = mpatches.FancyArrowPatch(posA=(0.25, 0.5), posB=(0.25, 1), hatch = 'o',mutation_scale=scale, color = arrow)
            ax.add_patch(AA)
            ax.set_axis_off()
        if k == 'l':
        ...
        if k in ['H', 'N', 'T', 'E', 'C', 'O', 'D']:
            ...
            if k == "obsE":
                print(8)
            onePlot(ax, dd=dd, plotVariable=k, color = color, low_h_cut = low_h_cut, high_h_cut = high_h_cut, plotData = filteredData)
        ...
        for windowA in simulation[stateVar]:
            ...
            for windowB in simulation[stateVar]:
                ...
                if sum(JaccardSum > 0) == 0:  # this means there are no IDs, so no union, its an NA
                    ...
                    jaccard.append(sum((simulation[stateVar][windowB] > 0) & (simulation[stateVar][windowA] > 0))/sum((simulation[stateVar][windowB]>0) + (simulation[stateVar][windowA]>0)))
...
def needsTreatmentHeat(data: dict, treatment: str):
    ...
    for index, simulation in data.items():
        ...
        fig.suptitle(f"Treeatment trajectories for all patients. Allocation: {tit_treatmentType(treatment)}. ({tit_systemType(treatment)})", size = 25)
        fig.show()
        return
...
def makeHist(errorData, jamHistAx, cmap = mpl.colormaps['plasma']):
    ...
    if treatment == 'hiPsi/basal' or treatment == 'lowPsi/basal':
        axNum, tit = 0, "First-Come, First-Served Scheduling"
    elif treatment == 'hiPsi/risk' or treatment == 'lowPsi/risk':
        axNum, tit = 1, "Risk-Scheduling"
    elif treatment == 'hiPsi/need' or treatment == 'lowPsi/need':
...
#Final JAM Figure
finalJamFigure, finalJamAxes = plt.subplots(nrows=1, ncols= 2, figsize=(10, 5), constrained_layout=True, facecolor='white',)
...
for treatment in treatments:
    ...
    #errorData = {0:dd}
    errorData = generateErrorData(params = parameters_recover, reps = 100, stateVariables=stateVariables, initialSeed=52920828945)
    ...
    if treatment == 'hiPsi/basal' or treatment == 'lowPsi/basal':
        axNum, tit, l,ll = 0, "First-Come, First-Served Scheduling", ["dotted","dotted","dotted","dotted"], ["solid", "dotted","solid", "dotted"]
    elif treatment == 'hiPsi/risk' or treatment == 'lowPsi/risk':
        axNum, tit, l,ll = 1, "Risk-Scheduling", ["dashed", "dashed","dashed", "dashed"],["dashdot", "dashed","dashdot", "dashed"]
    elif treatment == 'hiPsi/need' or treatment == 'lowPsi/need':
        axNum, tit, l,ll = 2, "Patient-Reported Outcomes Scheduling", ["solid","solid","solid","solid"], [(0, (5, 10)), (0,(3, 10, 1, 10)),(0, (5, 10)), (0,(3, 10, 1, 10))]
    ...
    seekAxes ={'grid': [jamTimeAx[1], ',', True, tit, l.copy()], 'double': [jamTimeAx2, ',', False, f'Care-seeking, {tit}', ll.copy()]} #The list contains: 0 axe, 1 fmt, 2 wheter to call legend, 3 legend text, 4 linestyle
    treatAxes ={'grid': [jamTimeAx[0], ',', True, tit, l.copy()], 'double': [jamTimeAx2twinx, ',', False, f'Treatment, {tit}', ll.copy()]} #The list contains: 0 axe, 1 fmt, 2 wheter to call legend, 3 legent text, 4 linestyle
    ...
    for key, seekAxis in seekAxes.items():
        seekAxis[0].plot(range(101), seekMean.mean(0), label = seekAxis[3], linestyle = seekAxis[4].pop(), color = cmap(0.6-(axNum*0.3)))
        seekAxis[0].errorbar(x=x_error_pos, y=seekMean.mean(0),
                             yerr=np.absolute(np.quantile(seekMean, q=[0.05, 0.95], axis=0) - seekMean.mean(0)), fmt= seekAxis[1],
    ...
    if treatAxes is not None:
        for key, treatAxe in treatAxes.items():
            treatAxe[0].plot(range(101), treatMean.mean(0), label = treatAxe[3], linestyle = treatAxe[4].pop(), color = cmap(0.6-(axNum*0.3)))
            treatAxe[0].errorbar(x=x_error_pos, y=treatMean.mean(0),
                                  yerr=np.absolute(np.quantile(treatMean, q=[0.05, 0.95], axis=0) - treatMean.mean(0)), fmt=treatAxe[1],
                                  ecolor=cmap(0.6-(axNum * 0.3)), elinewidth=0.2, color = cmap(0.6-(axNum * 0.3)))
    ...
    finalJamAxes[0].plot(range(101), treatMean.mean(0), label = finalAxConfigLEFT[3], linestyle=finalAxConfigLEFT[4].pop(), color=cmap(0.6 - (axNum * 0.3)))
    finalJamAxes[0].errorbar(x=x_error_pos, y=treatMean.mean(0),
                             yerr=np.absolute(np.quantile(treatMean, q=[0.05, 0.95], axis=0) - treatMean.mean(0)), fmt=finalAxConfigLEFT[1],
                             ecolor=cmap(0.6 - (axNum * 0.3)), elinewidth=0.2, color=cmap(0.6 - (axNum * 0.3)))
    ...
    #Participation
    finalJamAxesTwinx.plot(range(101), seekMean.mean(0), label=f'Seeking {tit.replace("Scheduling", "")}', linestyle="dotted",
                           color=cmap(0.6 - (axNum * 0.3)))
    ...
    finalJamAxes[1].legend(handles=handlesNedds + handlesPart, labels=labelsNeeds + labelsPart, loc='lower right', frameon=True)
